...
log_filter::detail::Condition ParseValue(std::string_view str, const ConditionType& type) {
...
log_filter::detail::Condition ParseCondition(const std::vector<std::string>& vec) {
    if (vec.empty()) {
...
Filter::FilterImpl::FilterImpl(const std::vector<std::vector<std::string>>& conditions) {
...
void Filter::FilterImpl::StartProcessing(std::istream& input, std::ostream& output) {
...
std::optional<LogRecord> Filter::FilterImpl::ParseAndCheckLine(std::string_view line) {
    // parse line
...
void Filter::FilterImpl::BufferToOutput(std::ostream& output) {
